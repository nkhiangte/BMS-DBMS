...
const HostelStaffPage: React.FC<HostelStaffPageProps> = ({ staff, onAdd, onEdit, onDelete }) => {
        ...
        <div className="bg-white rounded-xl shadow-lg p-4 sm:p-6 lg:p-8">
            ...
            <div className="space-y-10">
                <section>
                    ...
                    {wardens.length > 0 ? (
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                            {wardens.map(member => <StaffCard key={member.id} staffMember={member} onEdit={onEdit} onDelete={onDelete} />)}
                        </div>
                    ) : (
                ...
                <section>
                     ...
                     {otherStaff.length > 0 ? (
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                            {otherStaff.map(member => <StaffCard key={member.id} staffMember={member} onEdit={onEdit} onDelete={onDelete} />)}
                        </div>
                    ) : (
