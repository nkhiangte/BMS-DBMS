...
const TeacherFormModal: React.FC<TeacherFormModalProps> = ({ isOpen, onClose, onSubmit, staffMember, allStaff, gradeDefinitions }) => {
  ...
  const handleSubmit = (e: FormEvent) => {
    e.preventDefault();
    onSubmit(formData, assignedGrade ? (assignedGrade as Grade) : null);
  };
      ...
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-4xl flex flex-col" onClick={e => e.stopPropagation()}>
        <form onSubmit={handleSubmit} className="flex flex-col h-full">
         <div className="p-6 border-b">
             <h2 className="text-2xl font-bold text-slate-800">{staffMember ? 'Edit Teacher Details' : 'Add New Teacher'}</h2>
         </div>
          <div className="p-6 space-y-4 overflow-y-auto max-h-[70vh]">
            
            <AccordionSection title="1. Basic Personal Information" defaultOpen={true}>
              <div>
                ...
                <label className="block text-sm font-bold text-slate-800">Profile Photo</label>
                 <div className="mt-2 flex items-center gap-4">
                    <div className="w-24 h-24 rounded-full bg-slate-100 flex items-center justify-center overflow-hidden border">
                        {formData.photographUrl ? <img src={formData.photographUrl} alt="Teacher preview" className="w-full h-full object-cover" /> : <UserIcon className="w-16 h-16 text-slate-600" />}
                    </div>
                    <div className="flex flex-col gap-2">
                        <input type="file" ref={fileInputRef} onChange={handlePhotoChange} accept="image/*" className="hidden" id="teacher-photo-upload" />
            ...
            <AccordionSection title="2. Employment & Job Details">
              <div>
                 ...
                 <label htmlFor="assignedGrade" className="block text-sm font-bold text-slate-800">Class Teacher Assignment</label>
                  <select name="assignedGrade" id="assignedGrade" value={assignedGrade} onChange={(e) => setAssignedGrade(e.target.value)} className="mt-1 block w-full border-slate-300 rounded-md shadow-sm" disabled={formData.status !== EmploymentStatus.ACTIVE}>
                    ...
                    {GRADES_LIST.map(grade => {
                      ...
                      const assignedTeacher = classDef.classTeacherId ? allStaff.find(t => t.id === classDef.classTeacherId) : null;
                      const isAssignedToOther = assignedTeacher && (!staffMember || assignedTeacher.id !== staffMember.id);
                      const assignedTeacherName = assignedTeacher ? `${assignedTeacher.firstName} ${assignedTeacher.lastName}` : '';
                      return (<option key={grade} value={grade} disabled={isAssignedToOther}>{grade} {isAssignedToOther ? `(${assignedTeacherName})` : ''}</option>);
                    })}
                  </select>
              ...
              <div className="md:col-span-2 lg:col-span-2">
                <label htmlFor="subjectsTaught" className="block text-sm font-bold text-slate-800">Subjects Taught</label>
              ...
              <div>
                <label htmlFor="teacherLicenseNumber" className="block text-sm font-bold text-slate-800">License Number (Optional)</label>
             ...
             <AccordionSection title="3. Emergency Contact & Medical">
              <div>
             ...
             <AccordionSection title="4. Salary & Payroll Information (Optional)">
              <div>
               ...
               <div>
                <label htmlFor="basicSalary" className="block text-sm font-bold text-slate-800">Basic Salary</label>
          ...
          <div className="bg-slate-50 px-6 py-4 flex justify-end gap-3 rounded-b-xl border-t">
            ...
            <button type="submit" className="px-4 py-2 bg-sky-600 text-white font-semibold rounded-lg shadow-md hover:bg-sky-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-sky-500 transition">
              {staffMember ? 'Save Changes' : 'Add Teacher'}
            </button>
          </div>
        </form>
      </div>
